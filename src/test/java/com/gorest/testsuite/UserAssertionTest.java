...
public class UserAssertionTest extends TestBase {
    ...
    @Test
    //2. Verify the if the name of id = 5318 is equal to ”Gov. Rahul Nambeesan”
    public void test002() {
        response.body("[0].name", equalTo("Gov. Rahul Nambeesan"));
    }

    @Test
    //3.Check the single ‘Name’ in the Array list (Mandakini Mukhopadhyay)
    public void test003() {
        response.body("name", hasItem("Mandakini Mukhopadhyay"));
    }

    @Test
    //4.Check the multiple ‘Names’ in the ArrayList (Mohan Iyer, Uttam Trivedi)
    public void test004() {
        response.body("name", hasItems("Mohan Iyer", "Uttam Trivedi"));
    }
